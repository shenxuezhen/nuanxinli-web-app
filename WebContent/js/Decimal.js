...
Decimal.prototype.toString=function() 
{
    ...
    var zeroTail = _countZeroTail(sNumber);
    if (len - digit > 0){
        return sNumber.substring(0, len-digit).concat( ".",sNumber.substring(len-digit, len - zeroTail) );
    }else{
        var zeroAfterPoint = Array(digit - len + 1).join("0");
        return "0.".concat(zeroAfterPoint,sNumber.substring(0,len - zeroTail));
    }
}
